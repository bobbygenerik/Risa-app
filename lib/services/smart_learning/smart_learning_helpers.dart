...
double _calculateStringSimilarity(String a, String b) {
  ...
  int matches = 0;
  // ⚡ Bolt Optimization: Removed redundant `aUsed` array because outer loop
  // inherently advances when inner loop breaks.
  final bUsed = List<bool>.filled(bLen, false);

  // Find character matches
  // ⚡ Bolt Optimization: Use .codeUnitAt for zero-allocation char iteration
  // rather than expensive .split('') allocations.
...
Future<void> _saveLearningData() async {
  final prefs = await SharedPreferences.getInstance();
  await prefs.setString(SmartLearningEngine._learningDataKey, jsonEncode(_learningData));
}
...
Future<void> _saveProviderPatterns() async {
  ...
  await prefs.setString(SmartLearningEngine._providerPatternsKey, jsonEncode(data));
}
...
Future<void> _saveUserCorrections() async {
  ...
  await prefs.setString(SmartLearningEngine._userCorrectionsKey, jsonEncode(data));
}
}
