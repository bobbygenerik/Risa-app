...
class ProviderPattern {
  ...
  Map<String, dynamic> toJson() => {
        // Bolt: optimized enum serialization by using .name to avoid unnecessary string allocations via .toString().split('.').last
        'type': type.name,
        'pattern': pattern,
        'weight': weight,
      };
  ...
  factory ProviderPattern.fromJson(Map<String, dynamic> json) =>
      ProviderPattern(
        type: PatternType.values.firstWhere(
          // Bolt: optimized enum deserialization by using .name to avoid unnecessary string allocations via .toString().split('.').last
          (e) => e.name == json['type'],
