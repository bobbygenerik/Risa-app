...
class _M3UParsingResult {
  final List<Channel> channels;
  final String? epgUrl;

  _M3UParsingResult(this.channels, this.epgUrl);
}

/// Top-level function to parse M3U content in a separate isolate
_M3UParsingResult _parseM3UInBackground(String content) {
  final List<Channel> channels = [];
  ...
  if (rawLines.isNotEmpty && rawLines[0].contains('x-tvg-url=')) {
    ...
    if (urlMatch != null) {
      epgUrl = urlMatch.group(1);
      print('M3UParser: Found EPG URL: $epgUrl');
    }
  }
  ...
  return _M3UParsingResult(channels, epgUrl);
}

// Top-level function to parse VOD content in a separate isolate
Map<String, List<Content>> _parseVODInBackground(String content) {
  ...
  for (int i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#EXTINF:')) {
      ...
      final groupTitle = currentAttributes['group-title']?.toLowerCase() ?? '';
      final title = _extractChannelName(currentInfo);
      final isSeries = title.contains(RegExp(r'S\d+E\d+', caseSensitive: false)) ||
...
class M3UParserService {
  ...
  Future<List<Channel>> parseM3U(String content) async {
    final result = await compute(_parseM3UInBackground, content);
    _epgUrl = result.epgUrl;
  ...
  Future<Map<String, List<Content>>> parseVOD(String content) async {
    return await compute(_parseVODInBackground, content);
  }
}
...
String _cleanSeriesTitle(String title) {
  return title.replaceAll(RegExp(r'S\d+E\d+', caseSensitive: false), '').trim();
}
...
List<String>? _extractGenres(String? groupTitle) {
  ...
  if (lower.contains('sci-fi') || lower.contains('scifi')) genres.add('Sci-Fi');
  ...
  if (lower.contains('kids') || lower.contains('children')) genres.add('Kids');
