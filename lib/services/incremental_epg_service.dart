...
class IncrementalEpgService extends ChangeNotifier with WidgetsBindingObserver {
  ...
  static final RegExp _stationTokenRe = RegExp(
    r'^(?:[wkcb][a-z0-9]{2,4}|[a-z]{3,4}\d{1,2})$',
  );
  ...
  static String normalizeForFilter(String input) {
    // Check cache first
    final cached = _normalizeCache.remove(input);
    if (cached != null) {
      // Re-insert at end (most recently used) — O(1)
      _normalizeCache[input] = cached;
      return cached;
    }

    // LRU eviction: remove oldest entry if cache is full
    while (_normalizeCache.length >= _maxNormalizeCacheSize) {
      _normalizeCache.remove(_normalizeCache.keys.first);
    }
  ...
  void _resetLoadingState() {
    ...
    // Also collect channels that were batch-loaded but got empty results,
    // so they can be retried now that the parse is complete.
    ...
    // Remove the empty placeholders so ensureChannelsLoadedBatch will retry them
    ...
    _flushDeferredChannelRequests();
  }

  /// Retry any channel IDs that were requested during parsing/loading.
  void _flushDeferredChannelRequests() {
    ...
    while (retryCount < _maxRetries) {
      try {
        ...
        if (programFilePath == null ||
          ...
          // CRITICAL: Check hasLoadedPrograms, not just isNotEmpty
          final existingChannelCount = _programsByChannel.length;
          ...
          // If parse found channels but 0 programs (no XML errors), the cached
          // EPG data is stale — all programmes are too old for the time window.
          // Purge cache + backup and force a fresh download.
        ...
        // CRITICAL: Do this in background to avoid blocking UI
  ...
  Future<void> _refreshFromNetwork() async {
    ...
    try {
      await _downloadEpgIfNeeded(forceRefresh: false);
  ...
  static bool _shouldIncludeProgramme(
    ...
    // Also include past programs within 48 hours for timezone tolerance
    // and slightly-stale caches from providers with multi-day EPG data
    final pastCutoff = nowMs - (48 * 3600000); // 48 hours back
    if (endMs < pastCutoff) {
      ...
      if (endMs < earliest) {
        if (rejectStats != null) {
          rejectStats['beforeCatchup'] =
              (rejectStats['beforeCatchup'] ?? 0) + 1;
        }
        return false;
      }
    }
  ...
  static Future<Map<String, dynamic>> _parseEpgInIsolate(
    ...
    String? extractAttribute(String block, String name) {
      ...
      final match =
          regexes[0].firstMatch(block) ?? regexes[1].firstMatch(block);
      if (match == null) return null;
      return match.group(1);
    }

    String? extractTagText(String block, String tag) {
      // Cache regexes by tag name
  ...
  Set<String> _extractStationTokens(Set<String> tokens) {
    final stationTokens = <String>{};
    for (final token in tokens) {
      if (_stationTokenRe.hasMatch(token)) {
        stationTokens.add(token);
      }
    }
    return stationTokens;
  }

  bool _isInformativeMatchInput(String value) {
  ...
  String? _findBestEpgId(
    ...
    final useChannelIdForFuzzy = _isInformativeMatchInput(channelId);
    final channelIdNorm = useChannelIdForFuzzy
        ? EPGMatchingUtils.normalizeChannelName(channelId)
        : '';
    final channelIdTokens = useChannelIdForFuzzy
        ? EPGMatchingUtils.tokenize(channelId)
        : const <String>{};
    final stationTokens = <String>{
      ..._extractStationTokens(searchTokens),
      ..._extractStationTokens(channelIdTokens),
    };

    double bestScore = 0.0;
    EpgMatchCandidate? bestCandidate;

    Iterable<EpgMatchCandidate> candidatesToScan = _matchCandidates;
    if (stationTokens.isNotEmpty) {
      final stationCandidates = _matchCandidates.where((candidate) {
        for (final token in stationTokens) {
      ...
      if (stationCandidates.isNotEmpty) {
        candidatesToScan = stationCandidates;
      }
    }

    for (final candidate in candidatesToScan) {
      var score = EPGMatchingUtils.calculateMatchScore(
        searchName,
        candidate,
        playlistTokens: searchTokens,
        playlistNormalizedName: searchNameNorm,
      );
      if (useChannelIdForFuzzy &&
          channelIdNorm.isNotEmpty &&
          channelIdNorm != searchNameNorm) {
        final idScore = EPGMatchingUtils.calculateMatchScore(
          channelId,
          candidate,
          playlistTokens: channelIdTokens,
          playlistNormalizedName: channelIdNorm,
        );
        if (idScore > score) {
          score = idScore * 0.96;
        }
    ...
    // <70: Risky
    final threshold = stationTokens.isNotEmpty ? 45.0 : 65.0;
  ...
  Program? getCurrentProgram(String channelId,
    ...
    // Look up programs DIRECTLY instead of going through getProgramsForChannel
    // which does its own redundant resolution and can resolve to a different ID.
    var programs = _programsByChannel[epgId];
    if (programs == null || programs.isEmpty) {
      // Fallback: try the original channelId as a direct key
      programs = _programsByChannel[channelId];
    }
    if (programs == null || programs.isEmpty) {
      // Lazy load from DB in background to populate memory
      if (!_attemptedLoads.contains(epgId)) {
  ...
  Program? _findCurrentOrNextProgram(
    ...
    // Diagnostic: log why no program matched for this channel (only once per epgId)
    ...
    // Fallback: return the MOST RECENT past program.
    // This is critical for artwork lookups — a recently-ended show title
    // (e.g. "Good Morning America") is still valid for TMDB/TVDB backdrop search.
    Program? mostRecent;
  ...
  /// Channel IDs requested while parsing/loading — retried once parse finishes.
  ...
  Future<void> _loadProgramsFromDbBatch(List<String> epgIds) async {
    try {
      ...
      if (rows.isEmpty) {
        ...
        // Just return; the channels will be retried when _attemptedLoads is cleared.
        return;
      }
