/// Display and de-duplication helpers for the Live TV screen.
class LiveTvFormatters {
  LiveTvFormatters._();

  static final RegExp _articleRegExp = RegExp(r'^(the|a|an)\s+');
  static final RegExp _nonAlphaNumRegExp = RegExp(r'[^a-z0-9\s]');
  static final RegExp _multipleSpacesRegExp = RegExp(r'\s+');
  static final RegExp _epgRegExp = RegExp(r'\bEPG\b', caseSensitive: false);

  static String formatProgramTime(DateTime dt) {
    final hour = dt.hour == 0 ? 12 : (dt.hour > 12 ? dt.hour - 12 : dt.hour);
    final period = dt.hour < 12 ? 'AM' : 'PM';
    return '${hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')} $period';
  }

  /// Normalize titles for de-duplication: lowercase, strip articles, alphanumerics only.
  static String normalizeTitleForFilter(String title) {
    if (title.isEmpty) return title;
    var s = title.toLowerCase().trim();
    s = s.replaceAll(_articleRegExp, '');
    s = s.replaceAll(_nonAlphaNumRegExp, ' ');
    s = s.replaceAll(_multipleSpacesRegExp, ' ').trim();
    return s;
  }

  /// User-facing status line: replace "EPG" with "data".
  static String? replaceEpgWithData(String? s) {
    if (s == null) return null;
    if (!s.toLowerCase().contains('epg')) return s.trim();
    return s.replaceAll(_epgRegExp, 'data').trim();
  }
}
