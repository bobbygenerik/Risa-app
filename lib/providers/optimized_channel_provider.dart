...
class OptimizedChannelProvider extends ChangeNotifier with ThrottledNotifier {
  ...
  Future<void> loadChannels(String m3uUrl) async {
    ...
    try {
      debugLog(
          'OptimizedChannelProvider: Starting fast channel load from $m3uUrl');

      _channels.clear();
      _indicesByCategory.clear();
      _criticalDataLoaded = false;

      await _playlistLoader.loadFromUrl(m3uUrl, onProgress: (count) {
      ...
      debugLog(
          'OptimizedChannelProvider: Channels loaded (${_channels.length} channels)');
    } catch (e) {
