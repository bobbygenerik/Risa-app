...
class EpgBloc extends Bloc<EpgEvent, EpgState> {
  ...
  Future<void> _onInitialize(
      EpgInitialize event, Emitter<EpgState> emit) async {
  ...
  Future<void> _onLoadChannels(
      EpgLoadChannels event, Emitter<EpgState> emit) async {
    final currentState = state;
    if (currentState is! EpgReady) {
  ...
  Future<void> _onSetAllowedChannels(
      EpgSetAllowedChannels event, Emitter<EpgState> emit) async {
  ...
  Future<void> _onGetCurrentProgram(
      EpgGetCurrentProgram event, Emitter<EpgState> emit) async {
  ...
  Future<void> _onSetPlaybackMode(
      EpgSetPlaybackMode event, Emitter<EpgState> emit) async {
  ...
  Future<void> _onForceRefresh(
      EpgForceRefresh event, Emitter<EpgState> emit) async {
  ...
  Future<void> _onSetManualMapping(
      EpgSetManualMapping event, Emitter<EpgState> emit) async {
  ...
  Future<void> _onRemoveManualMapping(
      EpgRemoveManualMapping event, Emitter<EpgState> emit) async {
  ...
  List<Program> getProgramsForChannel(String channelId, {String? channelName}) {
    return _epgService.getProgramsForChannel(channelId,
        channelName: channelName);
  }
  ...
  List<MapEntry<String, double>> getSuggestedMatches(
      ...
      {int limit = 10}) {
    return _epgService.getSuggestedMatches(channelId, channelName,
        limit: limit);
  }
