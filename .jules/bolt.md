...
## 2026-05-23 - Optimize Enum Serialization Overhead
**Learning:** Using `enum.toString().split('.').last` causes unnecessary string allocations and list generation, which is a known performance anti-pattern.
**Action:** Use the built-in `enum.name` property (Dart 2.15+) which evaluates at compile time or is highly optimized, eliminating runtime string manipulation overhead.
