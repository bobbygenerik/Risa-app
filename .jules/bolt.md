...
## 2024-05-24 - [Avoid String Concatenation for RegExp Matching]
**Learning:** In hot paths (like `ProgramClassifier`), using string interpolation to concatenate multiple fields (e.g., `'$title $category'`) purely for a combined `RegExp.hasMatch()` check incurs measurable overhead due to intermediate string allocations.
**Action:** Use independent `.hasMatch()` checks combined with short-circuiting (`||`) operators to prevent intermediate string allocations and reduce garbage collection overhead.
