...
**Action:** Replace `string.split(separator).first` with `.indexOf(separator)` and `.substring(0, index)`. This avoids array allocations and stops processing early once the separator is found.## 2026-05-11 - Lazy String Allocation in Regex Matchers
**Learning:** Eagerly allocating substrings (e.g. `line.substring(start, end)`) inside custom matcher objects (like `_SimpleMatch`) creates unnecessary garbage collection overhead when the match isn't ultimately used or accessed.
**Action:** Store the `input` string along with `start` and `end` indices, and only slice the string lazily inside the `group()` getter.
