...
## 2025-03-21 - Optimized Iterable Allocation in URL Parsing
**Learning:** Chained `.where().toList()` iterables, even for small lists like URL path segments, generate measurable GC pressure when executed thousands of times during M3U parsing. The `.last` operation on these chains is particularly inefficient compared to a manual reverse loop.
**Action:** When finding the last matching element in a collection in a hot path, use a manual reverse `for` loop instead of `where(...).toList().last` to skip intermediate allocations.
