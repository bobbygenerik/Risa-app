...
**Action:** Replace `string.split(separator).first` with `.indexOf(separator)` and `.substring(0, index)`. This avoids array allocations and stops processing early once the separator is found.## 2024-02-14 - Enum Serialization Optimization
**Learning:** Using `enum.toString().split('.').last` for serializing/deserializing enums causes unnecessary string allocations and list generation.

## 2024-02-14 - Enum Serialization Optimization
**Learning:** Using `enum.toString().split('.').last` for serializing/deserializing enums causes unnecessary string allocations and list generation.
**Action:** Replaced with the built-in `enum.name` property, which is highly optimized and eliminates runtime string manipulation overhead.
