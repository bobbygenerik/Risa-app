...
**Action:** Replace `string.split(separator).first` with `.indexOf(separator)` and `.substring(0, index)`. This avoids array allocations and stops processing early once the separator is found.## 2024-05-18 - [Optimization of URL Parsing]
**Learning:** When replacing `.where(...).toList()` with manual iteration to prevent intermediate list allocations, always evaluate the difference between "hot" paths and "cold" error paths. Over-optimizing a cold `catch` block with fragile string indexing (`lastIndexOf`) to prevent an array allocation on a parsing failure is an anti-pattern. It introduces out-of-bounds `RangeError` risks on malformed strings for zero practical gain, violating the "don't optimize prematurely" and "don't sacrifice safety" directives.
**Action:** Confine micro-optimizations that involve manual string index manipulation solely to proven hot paths (like the successful `try` block). Leave error handling or edge-case fallback logic (`catch` blocks) using the safest, most readable standard library methods (e.g., `split()` and `.where()`). Always write a benchmark script testing empty, malformed, and edge-case inputs before suggesting manual index operations.
