    ...
    await for (final chunk in resp) {
      if (!checked) {
        ...
        if (buffer.length >= 2) {
          checked = true;
          print(
              'First bytes: ${buffer.sublist(0, buffer.length.clamp(0, 10))}');
          print(
              'Hex: ${buffer.sublist(0, buffer.length.clamp(0, 10)).map((b) => b.toRadixString(16).padLeft(2, "0")).join(" ")}');
          ...
          if (buffer[0] == 0x1f && buffer[1] == 0x8b) {
            ...
            final sampleUrl =
                "https://dragtvplus.lol/live/bobby/09052757/356971.ts";
